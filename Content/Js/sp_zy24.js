function BindingMenu(id, patCode, currentPage, isPC) {
    $.get("/TreatmentCenter/NutritionRecipeManagement/GetMeunList", { id: id, currentPage: currentPage }, function (dt) {
        ...
        $.each(dt, function (index, it) {
            ...
            html += '<div  class="trr layui-row"><span class="layui-col-xs12" style="text-align:left;" >';

            html += '<a herf="javascript(0);" title="' + tit + '" data-type="1" data-source=\'' + JSON.stringify(it) + '\' data-code="' + it.menuCode + '" class="code" style="margin-left:10px;text-decoration: none;font-weight:normal; cursor:pointer;  display:block; width:100%;" data-code="' + it.menuCode + '">' + fname + '</a>';
            html += '</span>';
            html += '<span class="layui-col-xs4" style="text-align:right; display:none;" >';

            html += '<a herf="javascript(0);" data-type="1" data-source=\'' + JSON.stringify(it) + '\' class="code" style="margin-right:10px;text-decoration: none;font-weight:normal; cursor:pointer;" data-code="' + it.menuCode + '">+</a>';
            html += '</span></div>';
        });
        $("#d2").append(html);
...
function bindCodeClick(patCode, isPC) {
    $(".code").bind("dblclick", function () {
        ...
        var color = '';
        if (mealid == '1') { color = 'red'; }
        if (mealid == '2') { color = 'yellow'; }
        if (mealid == '3') { color = 'blue'; }
        if (mealid == '4') { color = 'green'; }
        if (mealid == '5') { color = 'orange'; }
        if (mealid == '6') { color = 'cadetblue'; }
        if (type == 3) {
            ...
            if (istrue) {
                html += ' <tr lang=' + mealid + ' accesskey="10000" style="background-color:' + color + '"><td width="9%"><label style="display:none;">' + mealid + '</label>' + meal + '</td></td>';
                html += '<td width="20%">' + data.foodName + '</td>';
                html += '<td width="28%">' + data.foodName + '</td>';
                ...
                html += '<td><a herf="javascript(0);" data-id="' + daynum + '" class="fooddel" style="text-decoration: none;font-weight:normal; cursor:pointer; border: 1px solid #ccc; padding: 0 3px; padding-bottom: 3px; margin: 0;" onclick="bindDelClick(this);">×</a>&nbsp;&nbsp;';
                html += '<a herf="javascript(0);" class="foodMove" style="text-decoration: none; font-weight: normal; cursor: pointer; border: 1px solid #ccc; padding: 0 3px; padding-bottom: 3px; margin: 0;" onclick="bindMovelClick(this,0,\'' + patCode + '\',' + isPC + ');">↑</a>&nbsp;&nbsp;';
                html += '<a herf="javascript(0);" class="foodMove" style="text-decoration: none; font-weight: normal; cursor: pointer; border: 1px solid #ccc; padding: 0 3px; padding-bottom: 3px; margin: 0;" onclick="bindMovelClick(this,1,\'' + patCode + '\',' + isPC + ');">↓</a>';
                html += '</td></tr>';
            }
            ...
            if (istrue) {
                ...
                html += '<td width="28%">' + tempfullname + '</td>';
...
function CalculationNutrition(day, patCode,isPC) {
    ...
    var url = '/TreatmentCenter/ShareManagement/GetTAccommodationNutrientData'; 
    var data = { type: 0, days: daynum, contentStr: source, patCode: patCode, curDate: stDate, isPC: isPC }; 
